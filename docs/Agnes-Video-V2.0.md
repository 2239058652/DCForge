...
- 模型名称固定使用 `agnes-video-v2.0`。
- 视频生成是异步任务流程：先创建任务，再轮询查询结果。
- 创建视频任务使用 `POST https://apihub.agnes-ai.com/v1/videos`。
- 查询视频结果推荐使用 `GET https://apihub.agnes-ai.com/agnesapi?video_id=<VIDEO_ID>`。
- 兼容旧接入时可使用 `GET https://apihub.agnes-ai.com/v1/videos/<TASK_ID>`。
- 创建任务请求必须包含 `model` 和 `prompt`。
- 文生视频只需要文本提示词；图生视频使用顶层 `image` 字段传入单张图片 URL。
- 多图视频和关键帧动画使用 `extra_body.image` 传入多张图片 URL。
- 关键帧动画需要设置 `extra_body.mode: "keyframes"`。
- `num_frames` 必须小于或等于 `441`，并满足 `8n + 1`，例如 `81`、`121`、`161`、`241` 或 `441`。
- `frame_rate` 支持范围为 `1-60`。
- 创建任务响应会同时返回 `task_id` 和 `video_id`，新接入优先使用 `video_id` 查询。
- 查询结果中 `status` 为 `completed` 时，示例里的最终视频 URL 位于 `remixed_from_video_id`。
- 公开文档、示例和提示词中不要暴露真实 API Key，统一使用 `YOUR_API_KEY`。
...
创建视频任务后，响应中会返回 `video_id`。推荐使用 `video_id` 查询视频结果。
...
视频任务创建成功后，API 会返回任务信息。响应中会同时包含 `task_id` 和 `video_id`，其中 `video_id` 是推荐用于查询视频结果的 ID。
...
创建视频任务后，使用返回的 `video_id` 查询视频结果。建议轮询间隔为 `5s`。
...
```bash
curl -X GET "https://apihub.agnes-ai.com/agnesapi?video_id=video_xxxxxx" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://apihub.agnes-ai.com/agnesapi?video_id=<VIDEO_ID>&model_name=<MODEL>" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://apihub.agnes-ai.com/agnesapi?video_id=video_xxxxxx&model_name=agnes-video-v2.0" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

### 兼容方式：使用 `task_id` 查询

为了兼容旧版本，仍然可以使用 `task_id` 查询视频结果。该方式仍然支持，但新的接入建议使用 `video_id` 查询方式。
...
```bash
curl -X GET "https://apihub.agnes-ai.com/v1/videos/task_xxxxxx" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
| `seconds` | string | 视频时长，单位为秒 |
| `size` | string | 视频分辨率 |
| `remixed_from_video_id` | string | 示例中的最终视频 URL，仅在 `status` 为 `completed` 时可用 |
...
- 旧版 `task_id` 查询接口仍然支持。
- 示例中的最终视频 URL 位于 `remixed_from_video_id`，仅在 `status` 为 `completed` 时可用。
- `num_frames` 必须小于或等于 `441`。
- `num_frames` 必须满足 `8n + 1`，例如 `81`、`121`、`161`、`241` 或 `441`。
- 文生视频任务仅要求传入 `model` 和 `prompt`。
- 图生视频任务需要通过顶层 `image` 提供图片 URL。
