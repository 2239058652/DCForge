# Agnes 2.0 Flash API 指南

## LLM 调用规则摘要

- 模型名称固定使用 `agnes-2.0-flash`。
- Chat Completion Endpoint 为 `https://apihub.agnes-ai.com/v1/chat/completions`。
- 请求方法为 `POST`，请求体为 `application/json`。
- 请求必须携带 `Authorization: Bearer YOUR_API_KEY`。
- 基础请求必须包含 `model` 和 `messages`。
- `messages[].content` 可以是字符串，也可以是包含 `text` 和 `image_url` 的内容数组。
- 图片输入只支持公网可访问的图片 URL，使用内容块 `{"type": "image_url", "image_url": {"url": "..."}}`。
- 流式响应使用 `"stream": true`。
- 工具调用使用 `tools`，按需设置 `tool_choice`。
- OpenAI 兼容请求如需开启 Thinking，使用 `chat_template_kwargs.enable_thinking: true`。
- Anthropic 兼容请求如需开启 Thinking，使用 `thinking` 字段。
- 公开文档、示例和提示词中不要暴露真实 API Key，统一使用 `YOUR_API_KEY`。

## 模型概述

**Agnes 2.0 Flash** 是由 **Sapiens AI** 开发的快速、高效语言模型，面向智能体工作流、工具调用、编程任务、推理、多轮对话、图片理解以及高频生产环境应用场景设计。

Agnes 2.0 Flash 在 **Claw-Eval** 基准测试中取得了强劲表现，在 **General Leaderboard** 中排名第 **9**，**Pass^3 分数为 60.9%**，展现出在主流语言模型中较强的自主智能体能力。

该模型针对快速、可靠、低成本的语言生成、智能体任务执行和图片理解进行了优化。
...
| `temperature` | number | 否 | 控制输出随机性。较低值更确定，较高值更发散 |
...
Agnes 2.0 Flash 支持通过公网图片 URL 输入图片内容。开发者可以在同一个 `messages` 请求中同时传入文本指令和图片 URL，让模型基于图片进行理解、分析、问答或信息提取。
...
对于代码编写、调试、推理和 Agent 工作流，可以开启 Thinking 模式，以提升代码质量、任务拆解能力和问题解决效果。
...
`budget_tokens` 用于控制最大 Thinking token 预算。常见编码任务建议从 `2048` 开始设置；更复杂的调试、重构或多步骤 Agent 任务可以按需提高。
...
图片 URL 必须能被服务端公网访问。不要使用需要登录、Cookie、私有 Header 或本地网络才能访问的图片地址。

### 3. 图片内容结构错误

图片输入需要使用 `messages[].content` 数组，并在数组中提供 `image_url` 内容块。
